...
import { getGardens, deleteGardenById } from 'apiSdk/gardens';
...
function GardenListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('garden', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/gardens/view/${id}`);
    }
  };
  ...
  return (
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('garden', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            <Text as="h1" fontSize="2xl" fontWeight="bold">
              Garden
            </Text>
            <NextLink href={`/gardens/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('garden', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/gardens/edit/${record.id}`} passHref legacyBehavior>
                          <Button
